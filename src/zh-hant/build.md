...
- 執行 Bambu Studio。登入區域請選擇「中國內地（大陸）」，印表機請至少選擇 **Bambu Lab P1S** 和 **Bambu Lab P2S**，材料按預設配置即可。請勾選「安裝 Bambu 網路外掛」。
...
| **goBILDA** | C 型樑、方樑或薄樑，8mm REX 軸，各種單位化的連接件，連接功能完善。主要採用 M4 螺絲（官方 10mm），馬達多為 5203 系列。適用於馬達直傳的套件。 |
...
1. **零件採購**
   - FTC 官方器材供應商位於境外，採購工期長、價格高；非剛需材料可在國內平台尋找平替，兼顧成本與交付速度
2. **自製件製作**
   - 3D 列印件製作
   - 雷射切割板材加工
3. **硬體安裝**
...
| 印表機 | 噴嘴 | 耗材 |
|--------|------|------|
| P1S（未更換噴嘴） | 0.4 mm | PLA Basic |
| P1S（已更換噴嘴，專門列印碳材） | 0.6 mm | PETG-CF |
| P2S（兩台） | 0.4 mm | PLA Basic |
...
> [!info] 以上參數均為我隊的常用配置，如有特殊需求可自行修改。
