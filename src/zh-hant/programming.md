...
4. 等待檔案下載完成，在左側邊欄的「建置（Build）」頁籤或視窗右下透過進度條查看進度
...
> [!info] 手把操作（Driver）：參賽車輛使用 Xbox 手把操控，官方僅允許 Xbox 360 型號；Driver 由隊內選拔「誰行誰上」，擅長手把遊戲的同學有天然優勢。
...
> [!info] 現在寫程式並不困難：隊伍使用谷歌 Gemma 輔助程式設計，支援群組內共享使用與自訂人格提示詞。真正的核心能力是**讀懂 AI 生成的程式碼、明確修改方向並掌握 debug 能力**，無需從零手寫全部程式碼。

- 各類程式語言的核心邏輯（如 for 迴圈、while 迴圈）是通用的，僅語法存在差異；有 C++ 基礎的同學可以快速適應 Java 開發
