# Sensibilisation & Relations publiques

## 1. Informations de base

Voici les éléments complémentaires requis pour la division Sensibilisation & Relations publiques.

### Applications

- Navigateur web (Chrome, Edge ou Safari)
- LocalSend

### Comptes en ligne

- Adresse e-mail (@gmail.com ou @outlook.com recommandés)
- Compte officiel WeChat
- Bilibili
- Douyin
- Xiaohongshu (RED)
- YouTube
- TikTok
- X (Twitter)

---

## 2. Responsabilités

La division Sensibilisation & Relations publiques est le porte-voix et le corps diplomatique de l'équipe. Ses principales responsabilités comprennent :

### Cahier d'ingénierie

Le contenu du cahier d'ingénierie est rédigé par les équipes Mécanique et Programmation ; la division Relations publiques se charge de :

- **Mise en page** : la conception graphique et la mise en page du cahier d'ingénierie
- **Collecte de supports** : accumuler activement les supports de communication
- Organiser les documents d'ingénierie de la saison
- Rédiger le cahier d'ingénierie exigé pour les compétitions

### Collecte de supports de communication

- Contacter activement les équipes d'autres établissements pour organiser des matchs amicaux, des séances d'entraînement simulées et des réunions d'analyse du jeu de la nouvelle saison
- Contacter en priorité les lycées du district de Haidian disposant d'une équipe FTC
- Se concentrer sur l'accompagnement des nouvelles équipes : vous êtes désormais une équipe chevronnée, et les nouvelles équipes ont besoin de vous

### Maillots d'équipe et goodies

- Concevoir les maillots et le logo de l'équipe, afin que l'on voie « 32477 » et non « Beijing National Day Experimental School »
- Fabriquer des goodies : autocollants, badges, serviettes, écharpes, masques, etc.
- Objectif : que chaque équipe arbore nos goodies

### Sponsoring et ressources

- Rechercher et contacter des sponsors potentiels
- Rédiger les dossiers de demande de sponsoring et les plans commerciaux
- Entretenir les relations avec les sponsors
- Gérer le budget et les finances de l'équipe

### Promotion des STEM

- Organiser des activités d'éducation STEM au sein de la communauté
- Contacter les écoles et les entreprises locales
- Organiser des démonstrations de robots et des ateliers de découverte

### Échanges inter-équipes

- Établir des contacts avec d'autres équipes FTC
- Participer à des activités d'échange inter-équipes
- Partager expériences et ressources

### Gestion des réseaux sociaux

- Gérer les comptes de l'équipe sur les réseaux sociaux de chaque plateforme
- Publier les actualités de l'équipe et les informations sur les activités
- Produire des contenus de communication

---

## 3. Développement des sponsors

### L'essence du sponsoring : un échange d'intérêts

> [!info] Personne ne vous donnera d'argent sans contrepartie. Les sponsors fournissent des fonds et du matériel et attendent en retour de la visibilité et de la notoriété.

- Meilleurs sont vos résultats, plus grande est votre visibilité, et plus il est facile d'obtenir des sponsors.
- À ce stade, commencez par de petits sponsors : par exemple, sollicitez des usines d'électronique pour qu'elles fournissent des outils, des vis et d'autres consommables.
- Comment les contacter : envoyez des e-mails aux grandes entreprises et appelez directement les vendeurs Taobao.

### Plan commercial

Un plan commercial présenté à un sponsor doit inclure :

- La présentation de la compétition (ce qu'est la FTC et son influence)
- La présentation de l'équipe
- Les résultats historiques
- Les avantages de sponsoring par paliers : différents montants correspondent à différentes visibilités du logo

### Étude de cas : développement commercial de l'équipe FTC 27570 (BYNX)

- **Module son et lumière** : l'équipe FTC 27570 a développé KIRIN, un module son et lumière conçu spécialement pour la FTC et prenant en charge des effets lumineux personnalisables. La tarification par paliers intègre les numéros d'équipe : 1 unité pour ¥180, 4 unités pour ¥570 et 6 unités pour ¥787, où 570 et 787 correspondent respectivement aux numéros des équipes FTC 27570 et FTC 25787, gravant ainsi subtilement les numéros d'équipe dans l'esprit des acheteurs.
- **Servomoteurs personnalisés** : l'équipe a collaboré avec une usine de servomoteurs pour produire des servomoteurs dédiés sous sa propre marque, puis a développé un logiciel d'étalonnage associé ; elle vend servomoteurs, cartes de distribution d'alimentation et autres accessoires via son site web, les revenus des ventes servant à financer l'équipe.

### Étude de cas : l'équipe nationale chinoise du FGC

- FGC (First Global Challenge) est l'équivalent des Jeux olympiques de la robotique ; l'édition 2026 se tiendra à Incheon, en Corée du Sud, avec la participation de plus de 190 pays.
- Le plan commercial de l'équipe nationale chinoise du FGC comprend la présentation de la compétition, la présentation de l'équipe, les résultats historiques et les avantages correspondant aux différents paliers de sponsoring. D'après nos informations, cette équipe a obtenu le sponsoring de marques telles que Xiaomi et Huawei (via la marque Huakun Zhenyu).

---

## 4. Autres points

(voir les documents et manuels correspondants)
