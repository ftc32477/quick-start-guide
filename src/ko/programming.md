...
> [!info] 컨트롤러 조종(Driver): 로봇은 Xbox 컨트롤러로 조종하며, 공식적으로 Xbox 360 모델만 허용됩니다. Driver는 팀 내에서 '잘하는 사람이 맡는' 방식으로 선발되며, 컨트롤러 게임을 잘하는 학생은 유리합니다.
...
> [!info] 이제 코드를 쓰는 것은 어렵지 않습니다. 팀은 Google Gemma를 프로그래밍 보조 도구로 사용하며, 그룹 내 공유와 맞춤형 페르소나 프롬프트를 지원합니다. 진짜 핵심 능력은 **AI가 생성한 코드를 읽고, 수정할 방향을 알고, 디버깅하는 능력**입니다. 모든 코드를 처음부터 직접 쓸 필요는 없습니다.

- 프로그래밍 언어의 핵심 논리(for 반복문, while 반복문 등)는 언어마다 공통이며 문법만 다릅니다. C++ 기초가 있는 학생은 Java 개발에 빠르게 적응할 수 있습니다
