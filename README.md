...
1. **封面**：深色渐变背景（135°），内容放大并位于黄金分割点（内容中心 ≈ 38.2vh）；居中队徽、队伍徽章、"FIRST® Tech Challenge"、指南名（本地化）、学校，组团信息行距较大；底部居中"语言版本 / 版次日期"**两行**（如"简体中文版"+"2026年9月第3版"，位置略上移）
2. **封二（版权页）**：白底排版、内容置于页面下部，含完整书名、**版次（2026年9月第3版）**、**版本号（v1.3.0）**、**发布日期（2026年9月9日）**、语言版本、主编/编写人员、出品方与地址，以及完整法律声明；数据取自 `VERSIONS` 最新已发布条目，随发版自动更新
...
**页面布局参数：** A4 纸（210×297mm）；上下页边距各 2.54cm（1in）、左右各 3.18cm（1.25in）；页眉页脚字号均为 11pt（文字高度一致）；封面与封底边距为 0，封二为满版渲染、内边距与正文页边距一致（上下 1in、左右 1.25in）。**打印字号：正文 12px（与封三正文一致）、h1 24px、h2 19px、h3 15px、h4 13px、表格 12px、代码 11px、图注 10px，行距 1.6**（正文行距收窄，各级元素内边距同步收紧；正文标题为裸 h1–h4，须用裸元素选择器）。
...
| 新增语言版本：全书新语言译本 | 新增西班牙语（Español）版（v1.1.0 先例）、韩语（한국어）版（v1.2.0 先例） | 次版本 +1 → v1.4.0 | 第4版 |
| 常规更新：新增小节/附录/新页面 | 新增《工具清单》附录；招新内容并入（v1.3.0 先例） | 次版本 +1 → v1.4.0 | 第3版（日期随新） |
| 勘误：错别字/样式/小修正 | 人名与专有名词加注修正（v1.2.1 先例）、封面排版与工程化（v1.2.2 先例） | 修订 +1 → v1.3.1 | 第3版·第1次修订 |
