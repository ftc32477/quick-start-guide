...
2. **封二（版权页）**：白底排版，含书名、**版本号（v1.0.0）**、**版次（2026年8月第1版）**、**发布日期（2026年8月14日）**、语言版本、主编/编委、出品方与地址，以及完整法律声明；数据取自 `VERSIONS` 最新已发布条目，随发版自动更新
...
6. **封三（资源与更新页）**：在线版本、历史版本、开源仓库、意见反馈四个渠道链接 + **版本变更简表**（版本号/发布日期/主要变化，由 `VERSIONS` 已发布条目自动生成）
...
2. 简体中文版定稿后，再同步翻译到 `src/zh-hant/`（繁体中文）、`src/en-us/`（English (US)）和 `src/fr/`（Français）。
...
| zh-hant | 繁體中文 | 繁体中文（台湾正体用语，如程式設計、網路、雷射切割） |
| en-us | English (US) | 美式英语，**目录名必须为 en-us**（与 `en` 区分，明确美式变体） |
| fr | Français | 法语（法国），正式 vous 语体，遵循法语排版规范（« guillemets »、双标点前不换行空格等） |

- 语言代码统一采用 **BCP 47 脚本代码**：简体为 `zh-hans`、繁体为 `zh-hant`（不用区域代码 `zh-cn`/`zh-tw`），同时作用于目录名、语言键、HTML `lang`、PDF 文件名与线上网址路径。
